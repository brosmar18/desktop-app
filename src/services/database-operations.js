...
if (!window.dbOperations) {
  console.log("Initializing db operations service from JS file...")
  ...
  const cloneDatabase = async (sourceDb, targetDb, includeData) => {
    ...
    try {
      ...
      // Return success or error information from the result
      if (result.success) {
        return {
          success: true,
        ...
        return {
          success: false,
          error: result.error || 'Failed to clone database'
        };
      }
    } catch (error) {
  ...
  // Make sure the global object is created properly
  console.log('Initializing database operations service...');
  window.dbOperations = {
    cloneDatabase,
    backupDatabase,
    renameDatabase,
  ...
  console.log('Database operations service initialized successfully');
}